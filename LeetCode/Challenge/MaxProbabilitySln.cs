...
{
    class MaxProbabilitySln : ISolution
    {
        public double MaxProbability(int n, int[][] edges, double[] succProb, int start, int end)
        {
            Dictionary<int, List<Tuple<int, double>>> vertex = new Dictionary<int, List<Tuple<int, double>>>();
            for (int i = 0; i < n; i++)
            {
                vertex[i] = new List<Tuple<int, double>>();
            }

            for (int i = 0; i < edges.Length; i++)
            {
                int[] edge = edges[i];
                vertex[edge[0]].Add(new Tuple<int, double>(edge[1], succProb[i]));
                vertex[edge[1]].Add(new Tuple<int, double>(edge[0], succProb[i]));
            }

            double?[] p = new double?[n];
            PriorityQueue<Tuple<int, double>> pq =
                new PriorityQueue<Tuple<int, double>>(n,
                    Comparer<Tuple<int, double>>.Create((tuple, tuple1) => tuple1.Item2.CompareTo(tuple.Item2)));
            pq.Push(new Tuple<int, double>(start, 1.0));

            while (pq.Count > 0)
            {
                Tuple<int, double> temp = pq.Top;
                pq.Pop();

                int currentNode = temp.Item1;
                double currentProb = temp.Item2;
                ...
                p[currentNode] = currentProb;
                if (currentNode == end)
                {
                    break;
                }

                foreach (Tuple<int, double> tuple in vertex[currentNode])
                {
                    int adjNode = tuple.Item1;
                    double adjProb = tuple.Item2;

                    if (p[adjNode] == null)
                    {
                        pq.Push(new Tuple<int, double>(adjNode, currentProb * adjProb));
                    }
                }
            }
